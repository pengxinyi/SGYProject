namespace UFIDA.U9.LH.LHPubBP.AutoProgramBP
{
	...
	/// 定时更新委外退货
	/// </summary>	
	public partial class AutoCreateOCRtnRcv 
	{	
		internal BaseStrategy Select()
		{
			return new AutoCreateOCRtnRcvImpementStrategy();	
		}		
	}
	...
	/// 
	/// </summary>	
	internal partial class AutoCreateOCRtnRcvImpementStrategy : BaseStrategy
	{
		public AutoCreateOCRtnRcvImpementStrategy() { }

		public override object Do(object obj)
		{						
			AutoCreateOCRtnRcv bpObj = (AutoCreateOCRtnRcv)obj;
            GetOCRtnRcv(0);
            return "OK";
                ...
                else if (res.data!=null && res.data.order!=null)
                {
                    ...
                    foreach (var item in res.data.order)
                    {
                        ...
                        if (miscRcvTrans != null)
                        {
                            ...
                            if (org != null)
                            {
                                if (miscRcvTrans != null && miscRcvTrans.DocType.Code == "RCV93" && miscRcvTrans.Status == RcvStatusEnum.Approving)
                                {
                                    dataJson = JsonHelper.GetWDTUpdatePurRtnRcv(item, org, miscRcvTrans.ID);
                                    FailInfo fail = FailInfo.Finder.Find("DocNo=@DocNo and Org = @Org", new UFSoft.UBF.PL.OqlParam("DocNo", miscRcvTrans.DocNo),new UFSoft.UBF.PL.OqlParam("Org",org.ID));
                                    if(fail == null)
                                    {
